...
impl<T> Container<T> {
    pub fn new(elements: HashMap<(usize, usize), T>, width: usize, height: usize) -> Self {
        Container { elements, width, height }
    }

    pub fn grid_iter(&self) -> GridIterator<T> {
        GridIterator {
            grid: self,
            current_row: 0,
            current_col: 0,
    ...
    pub fn parse_from_lines(lines: &[String], transform: fn(char) -> T) -> Container<T> {
        ...
        Container { elements, width: lines[0].len(), height: lines.len() }
    }

    pub fn parse_from_str(input: &str, transform: fn(char) -> T) -> Container<T> {
        let mut elements = HashMap::new();
        input.chars().enumerate().for_each(|(idx, c)| {
                elements.insert((idx, 0), transform(c));
        });
        Container { elements, width: 0, height: 0 }
    }
